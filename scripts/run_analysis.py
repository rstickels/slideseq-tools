...
def main():
    ...
    try:
        ...
        submission_script = '{}/run.sh'.format(scripts_folder)
        call_args = ['qsub', '-o', output_file, '-l', 'h_vmem=90g', '-notify', '-l', 'h_rt=30:0:0', '-j', 'y', submission_script, 'generate_plots', manifest_file, library, scripts_folder]
        call(call_args)
        ...
        for l in lists:
            ...
            submission_script = '{}/run.sh'.format(scripts_folder)
            call_args = ['qsub', '-o', output_file, '-l', 'h_vmem=90g', '-notify', '-l', 'h_rt=36:0:0', '-j', 'y', submission_script, 'run_analysis_spec', manifest_file, library, scripts_folder, l]
            call(call_args)
