...
def main():
    ...
    try:
        ...
        for i in range(len(slice_id[lane])):
            ...
            call_args = ['qsub', '-o', output_file, '-l', 'h_vmem=75g', '-notify', '-l', 'h_rt=100:0:0', '-j', 'y', submission_script, 'run_barcodes2sam', manifest_file, commandStr, lane, slice_id[lane][i], scripts_folder]
            call(call_args)
