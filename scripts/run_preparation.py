...
def main():
    ...
    try:
        ...
        for lane in lanes_unique:
            ...
            call_args = ['qsub', '-o', output_file, '-l', 'h_vmem=90g', '-notify', '-l', 'h_rt=30:0:0', '-j', 'y', submission_script, 'run_processbarcodes', manifest_file, lane, scripts_folder]
            call(call_args)

        # Call run_mergebarcodes
        output_file = '{}/logs/run_mergebarcodes.log'.format(output_folder)
        submission_script = '{}/run.sh'.format(scripts_folder)
        call_args = ['qsub', '-o', output_file, '-l', 'h_vmem=12g', '-notify', '-l', 'h_rt=120:0:0', '-j', 'y', submission_script, 'run_mergebarcodes', manifest_file, scripts_folder]
        call(call_args)
